...
interface RevenueData {
    property_id: string;
    // Money is delivered as a string so the API contract carries the
    // exact, already-rounded value (no float precision drift). Parse to
    // a JS number only at the display layer.
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    // Backend already rounded to 2 decimals; parseFloat is just to get a Number for formatting.
    const totalNumber = parseFloat(data.total_revenue);
    const displayTotal = Math.round(totalNumber * 100) / 100;
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
            ...
            <div className="p-6">
                ...
                <div className="mt-4 h-6">
                    {Math.abs(totalNumber - displayTotal) > 0.000001 && showRaw && (
